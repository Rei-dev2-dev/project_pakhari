                    ...
                    @forelse($logs as $index => $log)
                        <tr class="hover:bg-slate-800/30 transition">
                            ...
                            <td class="py-3 px-4">
                                @if($log->photo_path)
                                    <button type="button"
                                        onclick="openLightbox('{{ Storage::url($log->photo_path) }}')"
                                        class="group relative block">
                                        <img src="{{ Storage::url($log->photo_path) }}"
                                            alt="Foto Bukti"
...
<!-- Lightbox Modal -->
<div id="photo-lightbox" class="fixed inset-0 z-50 hidden items-center justify-center bg-black/80 backdrop-blur-sm p-4">
    <div class="relative max-w-3xl w-full">
        <button onclick="closeLightbox()" class="absolute -top-10 right-0 text-white/70 hover:text-white transition text-sm font-semibold flex items-center gap-1.5">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
            </svg>
            Tutup
        </button>
        <img id="lightbox-img" src="" alt="Foto Bukti" class="w-full max-h-[80vh] object-contain rounded-2xl border border-slate-700 shadow-2xl">
    </div>
</div>
@endsection
