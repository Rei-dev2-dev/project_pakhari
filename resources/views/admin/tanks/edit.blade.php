...
@section('title', 'Edit Tangki - ' . $tank->name)
@section('page-title', 'Edit Tangki: ' . $tank->name)
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        ...
        <a href="{{ route('monitoring.show', $tank->id) }}" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 text-xs font-bold hover:bg-cyan-500/20 transition">
            <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
            </svg>
            <span>Buka Tampilan 3D</span>
        </a>
    </div>
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl space-y-5">
        <div class="border-b border-slate-800 pb-4">
            <h3 class="text-sm font-bold text-white">Ubah Parameter & Spesifikasi Tangki</h3>
            <p class="text-xs text-slate-400 mt-0.5">Perubahan kapasitas dan dimensi akan langsung mempengaruhi visualisasi 3D dan perhitungan persentase level air.</p>
        </div>
        ...
        <form action="{{ route('admin.tanks.update', $tank->id) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $tank->name) }}" required
                ...
                <div>
                    ...
                    <input type="text" id="code" name="code" value="{{ old('code', $tank->code) }}" required
            ...
            <div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <div>
                        ...
                        <input type="number" step="0.5" id="length_cm" name="length_cm" value="{{ old('length_cm', $tank->length_cm ?? 200.0) }}" required min="1"
                    ...
                    <div>
                        ...
                        <input type="number" step="0.5" id="width_cm" name="width_cm" value="{{ old('width_cm', $tank->width_cm) }}" required min="1"
                            class="w-full px-3 py-2 bg-slate-950/80 border border-slate-800 rounded-xl text-xs text-white font-mono focus:outline-none focus:border-cyan-500 dimension-input">
                    </div>

                    <div>
                        <label for="height_cm" class="block text-[11px] font-medium text-slate-400 mb-1">Tinggi (cm)</label>
                        <input type="number" step="0.5" id="height_cm" name="height_cm" value="{{ old('height_cm', $tank->height_cm) }}" required min="1"
                            class="w-full px-3 py-2 bg-slate-950/80 border border-slate-800 rounded-xl text-xs text-white font-mono focus:outline-none focus:border-cyan-500 dimension-input">
                    </div>

                    <div>
                        <label for="diameter_cm" class="block text-[11px] font-medium text-slate-400 mb-1">Diameter (cm)</label>
                        <input type="number" step="0.5" id="diameter_cm" name="diameter_cm" value="{{ old('diameter_cm', $tank->diameter_cm) }}" required min="1"
                            class="w-full px-3 py-2 bg-slate-950/80 border border-slate-800 rounded-xl text-xs text-white font-mono focus:outline-none focus:border-cyan-500 dimension-input">
                    </div>
                </div>
            </div>
            ...
            <div class="bg-cyan-950/30 border border-cyan-500/30 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div class="text-right shrink-0 bg-slate-950/60 px-4 py-2 rounded-xl border border-cyan-500/20">
                    <div class="text-lg font-extrabold text-cyan-400 font-mono" id="capacityDisplay">{{ number_format($tank->capacity_liters, 1, ',', '.') }} Liter</div>
                    <input type="hidden" id="capacity_liters" name="capacity_liters" value="{{ old('capacity_liters', $tank->capacity_liters) }}">
                </div>
            </div>
            ...
            <div>
                ...
                <textarea id="description" name="description" rows="2"
                    class="w-full px-3.5 py-2.5 bg-slate-950/80 border border-slate-800 rounded-xl text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500">{{ old('description', $tank->description) }}</textarea>
            </div>

            <!-- Status Active Toggle -->
            <div class="flex items-center space-x-2 pt-2">
                <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $tank->is_active) ? 'checked' : '' }}
